...
export function ResponseTimeDetailedChart({ data, timeRange }: ResponseTimeDetailedChartProps) {
  ...
  const minValue = Math.min(...data.filter((d) => d.responseTime > 0).map((d) => d.responseTime))
  const avgValue = Math.round(
    data.filter((d) => d.responseTime > 0).reduce((acc, d) => acc + d.responseTime, 0) /
      data.filter((d) => d.responseTime > 0).length,
  )
  ...
  const getTrend = () => {
    ...
    const recent =
      data
        ...
        .reduce((acc, d) => acc + d.responseTime, 0) / 5
    const previous =
      data
        .slice(-10, -5)
        .filter((d) => d.responseTime > 0)
        ...
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-2">
            ...
            <Badge variant="outline" className="border-primary/30 text-primary">
              {trend === "up" ? "Slower" : trend === "down" ? "Faster" : "Stable"}
            </Badge>
          </div>
        </CardTitle>
      </CardHeader>
      <CardContent className="space-y-6">
          ...
          <div className="flex items-end justify-between h-full space-x-1">
            {data.map((point, index) => (
              <div key={index} className="flex flex-col items-center flex-1 h-full justify-end group">
                <div
                  className={`${getStatusColor(point.status)} hover:opacity-80 transition-all duration-200 rounded-sm min-w-[2px] cursor-pointer relative`}
                  style={{
                    height: point.responseTime > 0 ? `${Math.max((point.responseTime / maxValue) * 100, 2)}%` : "2%",
                  }}
                  title={`${formatTimestamp(point.timestamp)}: ${point.responseTime > 0 ? `${point.responseTime}ms` : "Timeout"} (${point.status})`}
                >
                  {/* tooltip on hover */}
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-background/95 backdrop-blur-sm border border-border/50 rounded text-xs opacity-0 group-hover:opacity-100 transition-opacity duration-200 whitespace-nowrap z-10">
                    ...
                    <br />
                    {point.responseTime > 0 ? `${point.responseTime}ms` : "Timeout"}
                    <br />
        ...
        <div className="grid grid-cols-4 gap-4">
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <div className="font-mono font-semibold text-green-600 dark:text-green-400 text-lg">{minValue}ms</div>
            <div className="text-xs text-muted-foreground">Minimum</div>
          </div>
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <div className="font-mono font-semibold text-primary text-lg">{avgValue}ms</div>
            <div className="text-xs text-muted-foreground">Average</div>
          </div>
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <div className="font-mono font-semibold text-orange-600 dark:text-orange-400 text-lg">{maxValue}ms</div>
            <div className="text-xs text-muted-foreground">Maximum</div>
          </div>
          <div className="text-center p-3 bg-muted/30 rounded-lg">
            <div className="font-mono font-semibold text-blue-600 dark:text-blue-400 text-lg">
              {Math.round((data.filter((d) => d.status === "success").length / data.length) * 100)}%
            </div>
        ...
        <div className="flex justify-between text-xs text-muted-foreground px-4">
          <span>{formatTimestamp(data[0]?.timestamp || new Date().toISOString())}</span>
          <span>{formatTimestamp(data[data.length - 1]?.timestamp || new Date().toISOString())}</span>
        </div>
      </CardContent>
    </Card>
  )
